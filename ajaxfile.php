...
$pdostat = $conn->prepare("select task_id,date(DATE_SUB(deadline, INTERVAL 1 HOUR)),time(DATE_SUB(deadline, INTERVAL 1 HOUR)),date(deadline),time(deadline) from works");
$pdostat->execute();
...
foreach ($res as $row) {
    ...
    if($curdatetime >= $dbdatetime) {
        ...
        $tablecontent .= "<tr><td>$row[0]</td><td>$row[1]</td><td>$row[2]</td><td>$row[3]</td><td>$row[4]</td><td>$row[5]</td></tr>";
    }
}
echo $tablecontent;
?>
